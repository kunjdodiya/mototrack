...
- `src/features/auth/AuthGate.tsx` — React component; renders children only when authenticated, else renders `SignInScreen`; calls `syncWithCloud()` on first sign-in AND starts `liveSync` so every foreground/focus/resume/tick repulls remote rows across devices (unsubscribed on sign-out). Tracks the last signed-in user id in `localStorage` (`mototrack:lastUserId`) and runs `clearLocalUserData()` before the cloud pull when a *different* user signs in on the same device, so account A's rides/bikes/trips don't leak into account B's history and totals
- `src/features/auth/AuthGate.test.tsx` — renders sign-in when no session, children when session present; covers cross-user Dexie clear (different id triggers clear, same id skips, first-ever sign-in skips); mocks Supabase + storage/db
...
- `src/components/ForgotToStopSheet.tsx` — shared bottom-sheet used by both the live record screen ("Forgot to stop? Trim ride") and the ride recap ("Trim this ride"). Three synchronised range sliders drive a single `rewindSec` state: a **scrub** slider that runs from "keep full ride" to "rewind to ~1 min after start", plus **hours back** and **minutes back** dials for fine control. A mini Leaflet map at the top of the sheet renders the full route as a faded dashed line, the kept portion in solid orange, and a white-ringed orange dot at the cutoff so the rider can see *visually* where the ride will end before confirming. The slider ranges are clamped to the ride duration (minus a 1-min safety floor), so a user can never trim past the start. Preview pane shows the new end timestamp plus Kept vs Dropped (distance + duration). Copy is parameterised via `eyebrow` / `title` / `description` / `confirmLabel` props; anchoring is parameterised via `endReference` (defaults to `Date.now()` for the live path, set to `ride.endedAt` for the recap path so the rewind sliders count back from the original end). `onConfirm(cutoff)` is wired to `useRecorder.stopAt` on the live view and to `trimRide` + `pushRide` on the recap view. The portal-mounted map calls `invalidateSize()` on the next paint so leaflet picks up the sheet's final dimensions instead of leaving a blank panel until first interaction. The branded slider chrome (orange thumb on a gradient track) lives as `.moto-slider` in `src/index.css`
- `src/components/ForgotToStopSheet.test.tsx` — three sliders + cancel + confirm render, hours slider is capped + disabled when the ride is shorter than 1 hr, confirm fires with the right cutoff via the hour+minute sliders, the scrub slider drives the same cutoff, recap mode anchors on `endReference` (cutoff = `ride.endedAt − rewind`), Cancel fires `onClose`, the trim map mounts when `points.length > 1`
...
- `src/features/storage/db.ts` — Dexie v3: `rides` + `bikes` + `trips` tables. v2 added `bikes` + backfills `idleDurationMs` / `maxLeanAngleDeg` on pre-existing rides; v3 adds the `trips` table and a `tripId` index on `rides` so `where('tripId').equals(id)` is cheap. Also exports `clearLocalUserData()` — drops every user-scoped row (rides + bikes + trips) without touching the schema; called by `AuthGate` on a cross-user sign-in
- `src/features/storage/rides.ts` — CRUD: `saveRide`, `getRide`, `listRides`, `deleteRide`, `markSynced`, `renameRide`, `trimRide`. `renameRide(id, newName)` trims the input; an empty result clears the optional `name` field so the recap falls back to the formatted start date. Clears `syncedAt` so the next sync re-pushes the renamed row. `trimRide(id, newEndedAt)` retroactively shortens a saved ride: truncates track by `ts`, recomputes stats, clears `syncedAt` so the next sync re-pushes (upsert by id overwrites the Supabase row). The cutoff is clamped to `[startedAt, original endedAt]` so a silly value can never extend a ride or produce a negative duration
- `src/features/storage/rides.test.ts` — covers `trimRide` missing-id, happy-path trim + stats recompute, and both clamp edges; covers `renameRide` missing-id, trim-and-save happy-path, and empty/whitespace clears the name
- `src/features/storage/bikes.ts` — CRUD: `listBikes`, `getBike`, `addBike`, `renameBike`, `deleteBike`, `markBikeSynced`
- `src/features/storage/sync.ts` — `pushRide`, `pushBike`, `pushTrip`, `pushDeleteRide`, `pushDeleteBike`, `pushDeleteTrip`, `syncUnsyncedRides()`, `pullRemoteRides()`, `pullRemoteBikes()`, `pullRemoteTrips()`, `pullFromCloud()`, `syncWithCloud()`; scoped by Google user's `auth.uid()`. `AuthGate` calls `syncWithCloud()` on sign-in so rides, bikes, trips, and profile totals reconcile across every device signed into the same Google account. Trips are pushed before rides so a ride carrying a new `trip_id` references an already-persisted parent row. `pullFromCloud()` is the pull-only counterpart used by the live-sync loop on foregrounding. `pushDelete*` round-trip a Supabase delete (RLS pins it to `auth.uid()`) and return a boolean so callers can gate the local Dexie delete on cloud success — without that, the next pull resurrects the row. Pushes register themselves in an in-flight `pendingPushes` map keyed by `${table}:${id}`; `pushDelete` awaits any pending push for the same id before firing, so a delete that arrives while a fire-and-forget `pushBike` (or `pushRide` / `pushTrip`) is still on the wire can't race ahead, return "0 rows deleted", and let the late push resurrect the row on the next pull
- `src/features/storage/sync.test.ts` — unit tests for pull + two-way sync + `pushDelete*` happy-path/error returns + the in-flight push gate ordering; mocks Dexie + Supabase + session
- `src/features/storage/preferences.ts` — `readDefaultBikeId()` / `writeDefaultBikeId(id)` — localStorage-backed (`mototrack:default-bike-id`) per-device preference for the bike to pre-select on the Ride Now screen. Survives reloads, scoped per browser/device (deliberately not synced via Supabase — the rider may want a different default on their phone vs. their tablet)
- `src/features/storage/preferences.test.ts` — round-trip + null/empty clears + whitespace trim
...
- `src/components/RideSummary.tsx` — detail view styled to match the rest of the app (uppercase overline + ride title with an inline pencil button that swaps the title for an editable input + Save/Cancel; submit calls `renameRide(id, name)` + fire-and-forget `pushRide` so the renamed row syncs to Supabase, Escape cancels, an empty submit clears the name and lets the recap fall back to the start-date title, rounded-2xl map card with fade-up animation, ShareCard, a single "Add to a trip →" / "View trip →" link button (deep-links to `/trips` or `/trips/{tripId}` — all attach/detach/create flows live on the trip pages), gradient "Share to Story" button that triggers the 1080×1920 PNG compositor + native share sheet, border-white Delete, and a subtle "Forgot to stop? Trim ride →" link that opens `ForgotToStopSheet` anchored on `ride.endedAt`; confirm calls `trimRide(id, cutoff)` + fire-and-forget `pushRide` to re-upsert the shortened row in Supabase). Delete awaits `pushDeleteRide(id)` first and only removes the local Dexie row on cloud success — if the cloud delete fails we surface a "check your connection" alert so the next live-sync pull can't bring the deleted ride back. Loading and not-found states reuse the same header treatment
...
- `src/components/ProfileScreen.tsx` — gradient header with rider avatar + name + email, aggregate totals (count/distance/time/top speed/max lean), bike management (add/remove + per-bike "Set default" / "Unset" toggle that writes to `preferences.writeDefaultBikeId`; the default row is highlighted with a brand-gradient border + "Default" pill, and removing the default bike clears the preference too), legal-documents section, sign-out button
...
- `src/features/share/exportOverlayPng.ts` — 1080×1920 *transparent* PNG compositor for a single ride: route line (white halo + orange core) over the upper two-thirds of the canvas, plus three headline numbers (MOVING time / DISTANCE / AVG SPEED) with drop shadows so white text stays legible on any photo underneath. No speed graph — the overlay sits on a rider's own photo and the graph was visually noisy in that context. Every non-stroke, non-text pixel stays fully transparent
...
- `src/features/share/exportGlassPng.ts` — 1080×1920 *transparent* PNG compositor that mirrors the full Story poster (map route, title, bike chip, speed graph, 2-hero + 3×2 stats grid, bottom logo + footer) but paints every data module as a frosted-glass panel — translucent white fill, top highlight, bright inner-top border — with crisp text (drop shadow) on top. No map tiles, no base fill: every pixel outside a glass panel / route line / logo stays fully transparent so the rider drops their own photo behind the PNG. Canvas is scaled 2× for crisp text like `exportPng.ts`
- `src/features/share/exportGlassPng.test.ts` — throws on a single-point ride + produces a 2160×3840 physical-pixel PNG blob (1080×1920 logical); stubs fetch/createImageBitmap/toBlob under jsdom
- `src/components/ShareFormatPicker.tsx` — bottom-sheet modal that appears when a rider taps Share. Three tap targets: "Story poster" (existing 1080×1920 brand poster), "Glass poster" (same full layout as a transparent frosted-glass PNG), and "Transparent overlay" (just route + two numbers). Emits `onPick('poster' \| 'glass' \| 'overlay')` so the caller runs the right compositor. Rendered through a `createPortal` into `document.body` so the sheet escapes the page-transition `transform` on `<main>` — without this the sheet is pinned to the top of `<main>` instead of the viewport's bottom edge when the rider has scrolled down to tap Share. `ForgotToStopSheet` portals for the same reason
- `src/components/ShareFormatPicker.test.tsx` — renders all three choices + Cancel, and verifies each click wires to the right callback
...
- `src/components/CommunityScreen.tsx` — two-tab screen (Clubs / Club Manager) with an animated gradient pill indicator; **Clubs is the default tab**. Clubs panel shows a `LocationPicker` (free-text city/area/country, persisted in `localStorage`), "My clubs" pinned above a "Near [city]" (or "Discover" when no filter) list sourced from `listClubs({ cityLike })`. Discover hides clubs the user already joined and shows a `Join` pill on rows the user hasn't joined. Club Manager panel is the combined host surface: a "Host a ride" / "Start your own club" hero CTA (routes to `/community/events/new?clubId=…` when the user owns a club, `/community/clubs/new` otherwise), a "My clubs (managed)" list filtered to clubs where `createdBy === auth.uid()` with a per-row "Host a ride" shortcut + a sticky "New club" CTA, and a "My hosted rides" list (events whose `createdBy` matches the user) from `listUpcomingEventsForMyClubs()`
- `src/components/CommunityScreen.test.tsx` — header + default-tab (Clubs) + tab-switch; My-clubs-pinned-above-Discover when user has memberships; stored-location passed through to `listClubs({ cityLike })` with the "Near [city]" section label; save-location refetch; Club Manager "Create a ride" deep-link to owned club; Club Manager empty-state nudge when user manages no clubs
- `src/components/LocationPicker.tsx` — glass pill showing `Near [city]` / `Anywhere` with an inline expand-to-edit text input. Persists the trimmed value to `localStorage` under `mototrack:community-location` so the Clubs tab remembers the filter across reloads. Free-text on purpose — matches anything the user types (city, county, country) via a case-insensitive substring filter against `clubs.city`, so "California" and "USA" both work. Exports `readStoredLocation()` for the initial render. No browser geolocation: the Platform abstraction only exposes `watchPosition`, so a v1 text-only picker keeps the feature self-contained
- `src/components/LocationPicker.test.tsx` — unset vs "Near X" rendering, Save writes trimmed value + fires `onChange`, Clear removes the stored value, `readStoredLocation` returns trimmed non-empty values only
...
- `src/features/community/clubs.ts` — `listClubs({ cityLike? })`, `listMyClubs()`, `getClub(id)`, `createClub({ name, description?, city?, accent? })`, `joinClub(id)`, `leaveClub(id)`, `isMember(id)`. `listClubs` optionally appends an `ilike('city', '%needle%')` so the Discover tab can filter by free-text location. Remote `public.clubs` columns selected as a single `CLUB_COLUMNS` constant, mapped to the domain `Club` type with `createdAt` as epoch ms
- `src/features/community/clubs.test.ts` — list + **city-filter (`cityLike` substring, case-insensitive)** + myClubs + getClub (null path) + create (trim + default accent + null empties) + join/leave scoped to `(club_id, user_id)` + isMember truthiness
...
- `src/features/share/exportTripOverlayPng.ts` — 1080×1920 *transparent* PNG compositor for a whole trip: every session's route in per-session colors (same palette as `TripMap`) over the upper two-thirds of the canvas, plus three headline numbers (total moving time, total distance, trip-wide avg speed). No speed graph, no background, no brand fills — drops cleanly over any photo
