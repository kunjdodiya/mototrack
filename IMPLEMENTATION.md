...
- `src/components/ShareFormatPicker.tsx` — bottom-sheet modal that appears when a rider taps Share. Two tap targets: "Story poster" (existing 1080×1920 brand poster) and "Transparent overlay". Emits `onPick('poster' \| 'overlay')` so the caller runs the right compositor. Rendered through a `createPortal` into `document.body` so the sheet escapes the page-transition `transform` on `<main>` — without this the sheet is pinned to the top of `<main>` instead of the viewport's bottom edge when the rider has scrolled down to tap Share. `ForgotToStopSheet` portals for the same reason
