...
- `src/components/AdminScreen.tsx` — `/admin` screen. Four view states: loading, forbidden (non-admin), error (RPC threw), ready. Ready state renders stat-tile grids (users, active riders, rides + distance + duration, content counts), an inline 30-day signups bar chart (`SignupsChart` — pure flex/divs scaled against the max bucket), a top-riders leaderboard, and a recent-signups list. Refresh button re-fires the RPC without dropping back to a full loading state. Not on the bottom tab bar — reachable via the conditional "Owner console" card on `ProfileScreen` (rendered only when `checkIsAdmin()` returns true)
