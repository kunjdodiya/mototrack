...
- `src/components/ForgotToStopSheet.tsx` — bottom-sheet shown when a rider taps "Forgot to stop? Trim ride" on the live view. Preset chips (15 min · 30 min · 1-8 hr ago) plus a custom hours input; presets that exceed the ride duration are disabled; preview pane shows the cutoff timestamp plus Kept vs Dropped (distance + duration) so the rider can see exactly what they're saving before confirming; Save trimmed ride calls `onConfirm(cutoff)` which the RecordScreen wires into `useRecorder.stopAt`
- `src/components/ForgotToStopSheet.test.tsx` — presets + custom input render, presets past the ride duration are disabled, confirm fires with the right cutoff timestamp, Cancel fires `onClose`
