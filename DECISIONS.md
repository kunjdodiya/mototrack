...
- Entry points follow the "obvious next step" principle: History shows a Trips strip + a trip badge on any ride that belongs to one; the single-ride summary gets a single button — "Add to a trip →" deep-linking to `/trips` when unattached, or "View trip →" deep-linking to `/trips/{tripId}` when attached. All trip create / attach / detach UI lives on the trip pages themselves (`TripsList`, `NewTripScreen`, `TripDetailScreen` + `AddRidesToTripSheet`); the ride recap stays uncluttered with one tap target. No new bottom-tab entry — the feature sits under "My Rides" rather than competing for the fifth tab slot.
